...
#
#  - Create a new Fortran program (or adjust one of the provide example programs) to drive the FSPS engine.
...
#    \endverbatim
#
#  - Adjust your Fortran program to set appropriate global and local (pset) run-time parameter values that differ
...
#
#  - Run this script to convert the set of ".spec" files into a single SKIRT stored table file.
#
...
#    directories containing the built-in SKIRT resources.
#
...
#    replaced by some metallicity identifier in the filename. For example: "~/FSSP/FSSP_MyTemplates_*.spec".
#  - a file path for the stored table file, or "." to place the file in the current directory with a default name.
#
# \note The first argument **must** be included in quotes to keep the shell from expanding the file names.
# The filename pattern is used only for locating the files; the actual metallicity values are obtained from the file
