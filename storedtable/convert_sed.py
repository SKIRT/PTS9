...
# parameterized on metallicity and age, to stored table format. For info on FSPS, see Conroy, Gunn, & White (2009, ApJ)
# and Conroy & Gunn (2010, ApJ), and for info on the specific settings used, see the Note.txt file alongside the data.
# The function expects a sequence of input file paths and corresponding output file paths. Each input file path
# is actually interpreted as a template where the string "*" will be replaced by a metallicity index.
#
# The stored table output file contains specific luminosities in W/m normalized on an *initial* stellar population
# mass of 1 solar mass, in function of wavelength and parameterized on metallicity and age.
#
def convertFSPSSEDFamily(inFilePaths, outFilePaths):
    for inFilePath, outFilePath in zip(inFilePaths, outFilePaths):
        ...
        inpaths = sorted(glob.glob(inFilePath))
        ...
        # read the metallicity grid (from first line of each file)
        def readlogZ(inpath):
