...
## This function creates plots of media density cuts generated by the PlanarMediaDensityCutsProbe in a SKIRT
# simulation, allowing a visual comparison between the theoretical and spatially gridded medium density distribution
# for arbitrarily positioned grids.
#
# The function accepts a single Simulation instance and it assumes that the simulation includes exactly one
# PlanarMediaDensityCutsProbe. If this is not the case, the function does nothing.
#
# By default, the figures are saved in the simulation output directory with a filename that includes the simulation
# prefix, the probe name, and the medium indicator, and has the ".pdf" filename extension. The output directory
# can be overridden as described for the pts.utils.savePath() function. In interactive mode (see the
# pts.utils.interactive() function), the figures are not saved and are left open for display in notebooks.
#
def plotPlanarMediaDensityCuts(simulation, decades=5, *, outDirPath=None, figSize=(18, 6), interactive=None):

    # find the relevant probe
    probes = [ probe for probe in simulation.probes() if probe.type() == "PlanarMediaDensityCutsProbe" ]
    if len(probes) != 1:
        return
    probe = probes[0]
